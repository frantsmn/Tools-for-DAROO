...
$(function () {
	...
	function get_type() {
		...
		if (typeof Product !== 'undefined')
			obj = Product.get('type') === "product" && $('div.accordion-header').length ? {
				type: "product",
				str1: "Редактировать разводящую карточку",
				str2: "Разводящая карточка"
			} :
			obj = Product.get('type') === "product" ? {
				type: "product",
				str1: "Редактировать карточку товара",
				str2: "Карточка товара"
			} :
			obj = Product.get('type') === "price" ? {
				type: "price",
